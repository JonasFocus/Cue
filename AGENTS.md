...
./scripts/migrate.sh --check       # require no pending, changed, or orphaned files
```
...
Neon point-in-time restore is the only current recovery mechanism. Before any
schema release, confirm the plan's retention window and create a recoverable
snapshot or branch; record that it can be restored before applying irreversible
DDL. There is no independent `pg_dump` backup, so do not describe production as
recoverable until an owner has tested a restore and configured a second copy.
...
machine, gitignored. Everything since is in Neon. A schema migration must be
backward-compatible with the currently deployed app, and destructive changes
need a separate, explicitly reviewed follow-up release after the code no longer
reads the old data.
